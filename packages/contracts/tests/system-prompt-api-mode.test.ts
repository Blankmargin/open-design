...
describe('composeSystemPrompt — API mode (#313)', () => {
  ...
  describe('API mode (streamFormat: plain)', () => {
    ...
    it('allows multi-file artifacts for multi-page API/BYOK prototypes', () => {
      ...
      expect(prompt).toContain('Prototype auth is for clickable demos, not real security');
      ...
      expect(prompt).toContain('`operator/ops2024`');
      expect(prompt).toContain('Keep the flow sandbox-runnable');
      expect(prompt).toContain('every protected HTML page AND `login.html` must load the shared auth script');
      expect(prompt).toContain('`login.html` MUST include `<script src="js/auth.js"></script>` before the inline submit handler');
      expect(prompt).toContain('MUST call `event.preventDefault()`');
      ...
      expect(prompt).toContain('edit that child HTML file from its current source');
