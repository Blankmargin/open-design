...
describe('composeSystemPrompt — API mode (#313)', () => {
  ...
  describe('API mode (streamFormat: plain)', () => {
    ...
    it('allows multi-file artifacts for multi-page API/BYOK prototypes', () => {
      ...
      expect(prompt).toContain('every protected HTML page AND `login.html` must define/load the shared auth script');
      ...
      expect(prompt).toContain('Demo auth persistence MUST set `window.name` unconditionally');
