...
- When adding login/auth to an existing prototype, preserve the existing app entry as \`index.html\` and create the new login screen as \`login.html\`. Update \`index.html\` to redirect unauthenticated users to \`login.html\`, and update \`login.html\` to redirect successful login back to \`index.html\`. Do not put the login page in \`index.html\` or rename the existing app to \`admin.html\` unless the user explicitly asks for that file structure. The auth flow must actually run in Open Design's sandboxed preview: every protected HTML page must load the shared auth script, and \`login.html\` must define/load \`handleLogin(event)\` before form submission. Preview iframes polyfill \`localStorage\` / \`sessionStorage\` in memory — calls succeed, but data is lost when navigating between HTML files. Auth persistence MUST set \`window.name\` unconditionally as the primary cross-page mechanism (it survives same-frame navigation), then optionally write \`localStorage\` inside try/catch for same-page reads. Do NOT put the \`window.name\` write inside a \`catch\` block — the in-memory polyfill makes \`localStorage\` succeed so the \`catch\` never runs. Correct persist pattern: \`window.name='authed'; try{localStorage.setItem('a','1')}catch(_){}\`. Correct guard pattern: check \`window.name\` first, then \`localStorage\` in try/catch.
...
function renderMetadataBlock(
  ...
  if (metadata.kind === 'prototype' || metadata.kind === 'template' || metadata.kind === 'other') {
    ...
    lines.push(
      '- **auth-flow runnable rule**: login/auth is only complete when it works across files in the preview. Put shared guard/logout/login helpers in a common JS file or include equivalent script on every affected page. `login.html` must submit through a real `handleLogin(event)` or equivalent handler that persists auth and redirects to `index.html`; protected pages must check auth and redirect to `login.html`; logout must clear auth and redirect. Preview iframes polyfill `localStorage` / `sessionStorage` in memory — calls succeed, but data is lost when navigating between HTML files. Auth persistence MUST set `window.name` unconditionally as the primary cross-page mechanism (it survives same-frame navigation), then optionally write `localStorage` inside try/catch for same-page reads. Do NOT put the `window.name` write inside a `catch` block — the in-memory polyfill makes `localStorage` succeed so the `catch` never runs. Correct persist pattern: `window.name="authed"; try{localStorage.setItem("a","1")}catch(_){}`. Correct guard pattern: check `window.name` first, then `localStorage` in try/catch.',
    );
    lines.push(
