...
- For a new multi-page or multi-menu prototype, a final \`<artifact type="application/vnd.open-design.files+json">...</artifact>\` block containing JSON with \`entry\` and \`files[]\`. Include \`index.html\` as \`entry\`; every item in \`files[]\` MUST be an object with string \`name\` and string \`content\` fields, for example \`{ "name": "devices.html", "content": "<!doctype html>..." }\`; include the initial child page/CSS/JS files needed for the prototype. \`index.html\` must be a lightweight launcher/table of contents whose job is to link the child pages; do not place full screen UI, dashboards, forms, overview/KPI boards, charts, tables, filters, topbar business actions, or page-specific content in \`index.html\`. If the product needs an overview/dashboard screen, create a child file such as \`overview.html\` or \`dashboard.html\` and link it from \`index.html\`. Every menu item in \`index.html\` MUST be a real same-frame file link such as \`<a href="devices.html">设备管理</a>\`; do not use \`target="_blank"\`, \`window.open\`, \`onclick\`, \`data-page\`, \`href="#..."\`, \`switchPage(...)\`, hash routers, hidden page sections, or tabbed single-page routing inside \`index.html\`.
- For follow-up edits to an existing project, preserve all existing files and include only created or changed files in \`files[]\`. Files omitted from a multi-file artifact remain unchanged on disk; do not regenerate every page unless the user explicitly asks for a redesign or full rebuild. When the user asks to change an existing page/screen, edit that child HTML file from its current source instead of folding the change into \`index.html\`. Edit \`index.html\` only when the change adds/removes/renames a child page link or changes launcher-level navigation. If a navigation click is broken, fix the relevant \`href\` to point at the child file; do not replace it with JavaScript routing. When a follow-up adds or changes a global shell control shared with the sidebar/topbar/header (for example 大屏驾驶舱, notifications, help, profile, logout, search, tenant switcher, or other controls at the same level as existing topbar actions), update every child HTML file that renders that shell, or move the shell control into a shared JS/CSS asset loaded by every page. Do not add a global control only to the currently active page. When an existing HTML file must change, start from its current source and preserve unrelated markup, styles, scripts, and layout while making the smallest necessary change.
...
- All visible interactive controls must be runnable in the preview, not merely decorative. Before emitting the artifact, audit every \`<a>\`, \`<button>\`, \`<form>\`, menu item, tab, filter, modal trigger, drawer trigger, delete/edit/save/export action, pagination control, and login/logout control. Each must either navigate to a real target, submit through a defined handler, update visible DOM/state, open/close the intended UI, copy/download/export something, or be explicitly \`disabled\` with disabled styling. Do not emit dead controls such as \`href="#"\`, \`href="javascript:void(0)"\`, buttons with no defined handler/binding, inline handlers that call missing functions, forms without submit handling, or controls that only show a toast while the core action does nothing.
- When adding login/auth to an existing prototype, preserve the existing app entry as \`index.html\` and create the new login screen as \`login.html\`. Update \`index.html\` to redirect unauthenticated users to \`login.html\`, and update \`login.html\` to redirect successful login back to \`index.html\`. Do not put the login page in \`index.html\` or rename the existing app to \`admin.html\` unless the user explicitly asks for that file structure. Prototype auth is for clickable demos, not real security: do NOT implement real backend login, OAuth, cookies, CSRF, JWT refresh, password hashing, external auth redirects, or API calls unless the user explicitly asks for production auth. Use a mock front-end-only flow that accepts any non-empty email/username + password, shows inline validation for empty fields, sets demo auth state, and redirects. Do NOT create fixed credential arrays or whitelist checks such as \`VALID_CREDENTIALS\`, \`DEMO_USERS\`, \`admin/admin123\`, \`operator/ops2024\`, or \`username === ... && password === ...\`; those make the prototype feel broken when the user types any other demo credential. The auth flow must actually run in Open Design's sandboxed preview: every protected HTML page AND \`login.html\` must define/load the shared auth script before any inline code calls \`doLogin(...)\`, \`handleLogin(event)\`, \`checkAuth()\`, or \`doLogout()\`. If \`doLogin\` / \`handleLogin\` lives in \`js/auth.js\`, \`login.html\` MUST include \`<script src="js/auth.js"></script>\` before the inline submit handler, and the login form submit handler MUST call \`event.preventDefault()\` or the form \`onsubmit\` MUST return false before redirecting. Preview iframes polyfill \`localStorage\` / \`sessionStorage\` in memory — calls succeed, but data is lost when navigating between HTML files. Demo auth persistence MUST set \`window.name\` unconditionally as the primary cross-page mechanism (it survives same-frame navigation), then optionally write \`localStorage\` inside try/catch for same-page reads. Do NOT \`return\` after writing \`localStorage\` or \`sessionStorage\`; the sandbox shim can make those writes appear successful while still losing state across HTML navigation. Do NOT put the \`window.name\` write inside a \`catch\` block — the in-memory polyfill makes \`localStorage\` succeed so the \`catch\` never runs. Correct persist pattern: \`window.name='authed'; try{localStorage.setItem('a','1')}catch(_){}\`. Correct guard pattern: check \`window.name\` first on every guard call, before any \`localStorage\` or \`sessionStorage\` read: \`if (window.name === AUTH_KEY) return true; try{ if(localStorage.getItem(AUTH_KEY)==="1") return true }catch(_){}; return false;\`. Do NOT put \`window.name\` checks only inside \`catch\`, and do NOT \`return localStorage... || sessionStorage...\` before checking \`window.name\`. Do NOT rely on \`?auth=1\`, \`?redirect=...\`, \`URLSearchParams(window.location.search)\`, query strings, or hashes to carry auth between \`.html\` files; Open Design srcDoc previews strip query/hash during file navigation. After login, set \`window.name\` first and navigate in the same frame to a plain file path such as \`location.replace("index.html")\`. Do NOT use \`window.top.location\`, \`window.parent.location\`, \`window.open\`, \`target="_top"\`, \`target="_parent"\`, or \`target="_blank"\` for internal auth navigation; those bypass or break the sandbox file-switch bridge.
...
function renderMetadataBlock(
  ...
  if (metadata.kind === 'prototype' || metadata.kind === 'template' || metadata.kind === 'other') {
    lines.push(
      '- **screen-file-first rule**: each distinct user-facing screen or surface MUST be delivered as its own HTML file unless the user explicitly asks for a single-page scroll or single-file artifact. Do not combine landing pages, product app screens, dashboards, history, pricing, settings, mobile app, tablet app, desktop app, or OS widget surfaces into one long page. Use `index.html` only as a lightweight launcher/table of contents that links to screen files when more than one screen exists; it may include a concise product title and navigation links, but it must not contain full screen UI, overview/KPI boards, dashboards, forms, page-specific tables, charts, filters, topbar business actions, or hidden sections for the child pages. If the product needs a homepage/overview dashboard, create it as a child file such as `overview.html` or `dashboard.html` and link it from `index.html`.',
    );
    lines.push(
      '- **index-entry rule**: first-turn prototypes with multiple menus, modules, pages, routes, or screens MUST include an `index.html` entry file. `index.html` is the stable launcher/directory and must link to child pages such as `customers.html`, `operations.html`, `analytics.html`, `devices.html`, `logs.html`, `login.html`, or other domain-specific pages. Put the real screen layout, interaction markup, and page-specific styling in those child files and shared `css/` files, not in `index.html`. Do not ship only a child page or an unnamed artifact for multi-page work.',
    );
    lines.push(
      '- **index-link contract**: links from `index.html` to child pages MUST be normal same-frame anchors with concrete project-relative `.html` hrefs, for example `<a href="devices.html">设备管理</a>`. Do not add `target="_blank"` to internal project HTML links and do not use `window.open()` for child pages; OD previews run inside an iframe and internal navigation must replace the preview frame. Do not implement index navigation with `onclick`, `data-page`, `switchPage()`, `preventDefault()`, `location.hash`, hashchange listeners, tabs, hidden `.page-section` panels, or a client-side router. If the brief lists menus such as 客户管理 / 运营管理 / 数据分析 / 设备管理 / 日志管理, `index.html` should contain those links and the corresponding child files should contain the actual page UI.',
    );
    lines.push(
      '- **shared-asset rule**: multi-page prototypes should use shared assets such as `css/app.css` and optional `js/app.js` for common shell, tokens, sidebar, and controls. Avoid duplicating large inline `<style>` blocks across every child page unless the user explicitly asks for self-contained single files.',
    );
    lines.push(
      '- **follow-up child-page edit rule**: in later turns, treat `index.html` as navigation glue only. If the user asks to change a specific page/screen or any feature that belongs on a child page, update the relevant child HTML/CSS file. If the user asks to add a page, create a new descriptive child HTML file and update `index.html` only to add its link. Do not move child-page content into `index.html` during follow-up edits.',
    );
    lines.push(
    ...
    lines.push(
      '- **shared-shell consistency rule**: when multiple child pages share the same sidebar/topbar/header/app shell, a follow-up that adds or changes a shell-level control must update that shell everywhere. Examples: 大屏驾驶舱 beside notifications/help, notifications, help, profile, logout, global search, tenant switcher, or any button at the same level as existing topbar actions. Either include every affected child HTML file in `files[]`, or create/update a shared `js/shell.js` / `css/app.css` asset and ensure every page loads it. Do not add the control only to `index.html` or only to the active page, because it will disappear after navigating to another module.',
    );
    lines.push(
      '- **interactive-control runnable rule**: every visible interactive control must work in the preview before you emit the artifact. Real links must point to concrete files/anchors that exist; buttons, menu actions, filters, pagination, tabs, modals, drawers, copy/export/download/save/delete/edit actions, and forms must have defined handlers or event bindings that update visible DOM/state, navigate, submit, copy/download/export, or open/close the intended UI. Controls that are intentionally unavailable must be `disabled`. Do not ship `href="#"`, `javascript:void(0)`, undefined inline handlers, unbound buttons, forms with no submit path, or toast-only fake actions.',
    );
    lines.push(
      '- **demo-auth rule**: prototype login/auth screens are for clickable demos, not real security. Unless the user explicitly asks for production auth, do NOT implement real backend login, OAuth, cookies, CSRF, JWT refresh, password hashing, external auth redirects, or API calls. Use a mock front-end-only flow that accepts any non-empty email/username + password, shows inline validation for empty fields, sets demo auth state, and redirects. Do NOT create fixed credential arrays or whitelist checks such as `VALID_CREDENTIALS`, `DEMO_USERS`, `admin/admin123`, `operator/ops2024`, or `username === ... && password === ...`; those make the prototype feel broken when the user types any other demo credential. Keep it visually credible, but treat credentials and roles as sample data.',
    );
    lines.push(
      '- **auth-flow runnable rule**: login/auth is only complete when it works across files in the preview. Put shared guard/logout/login helpers in a common JS file or include equivalent script on every affected page. `login.html` must submit through a real `handleLogin(event)` or equivalent mock handler that calls `event.preventDefault()` (or returns `false` from `onsubmit`), persists demo auth, and redirects to `index.html`; protected pages must check demo auth and redirect to `login.html`; logout must clear demo auth and redirect. Every page that calls a shared auth function must load that shared auth script first: if `doLogin`, `handleLogin`, `checkAuth`, or `doLogout` lives in `js/auth.js`, then `login.html` MUST include `<script src="js/auth.js"></script>` before its inline submit handler, and protected pages must include it before `checkAuth()` / logout bindings. Preview iframes polyfill `localStorage` / `sessionStorage` in memory — calls succeed, but data is lost when navigating between HTML files. Demo auth persistence MUST set `window.name` unconditionally as the primary cross-page mechanism (it survives same-frame navigation), then optionally write `localStorage` inside try/catch for same-page reads. Do NOT `return` after writing `localStorage` or `sessionStorage`; the sandbox shim can make those writes appear successful while still losing state across HTML navigation. Do NOT put the `window.name` write inside a `catch` block — the in-memory polyfill makes `localStorage` succeed so the `catch` never runs. Correct persist pattern: `window.name="authed"; try{localStorage.setItem("a","1")}catch(_){}`. Correct guard pattern: check `window.name` first on every guard call, before any `localStorage` or `sessionStorage` read: `if (window.name === AUTH_KEY) return true; try{ if(localStorage.getItem(AUTH_KEY)==="1") return true }catch(_){}; return false;`. Do NOT put `window.name` checks only inside `catch`, and do NOT `return localStorage... || sessionStorage...` before checking `window.name`. Do NOT rely on `?auth=1`, `?redirect=...`, `URLSearchParams(window.location.search)`, query strings, or hashes to carry auth between `.html` files; Open Design srcDoc previews strip query/hash during file navigation. After login, set `window.name` first and navigate in the same frame to a plain file path such as `location.replace("index.html")`. Do NOT use `window.top.location`, `window.parent.location`, `window.open`, `target="_top"`, `target="_parent"`, or `target="_blank"` for internal auth navigation; those bypass or break the sandbox file-switch bridge.',
    );
    lines.push(
