...
- When adding login/auth to an existing prototype, keep \`index.html\` as the app entry and add the login screen as a separate \`login.html\`; protected pages redirect to \`login.html\` when not authenticated, and \`login.html\` redirects back to \`index.html\` on success. Do not move the login UI into \`index.html\` or rename the existing app. Prototype auth is for clickable demos, not real security: do NOT implement real backend login, OAuth, cookies, CSRF, JWT, password hashing, external auth redirects, or API calls unless the user explicitly asks. Use a mock front-end-only flow that accepts any non-empty email/username + password, shows inline validation for empty fields, sets a demo auth flag, and redirects. Do NOT create fixed credential arrays or whitelist checks such as \`VALID_CREDENTIALS\`, \`DEMO_USERS\`, \`admin/admin123\`, \`operator/ops2024\`, or \`username === ... && password === ...\`; those make the prototype feel broken when the user types any other demo credential. Keep the flow sandbox-runnable: every protected HTML page AND \`login.html\` must load the shared auth script before any inline code calls it, so \`login.html\` MUST include \`<script src="js/auth.js"></script>\` before the inline submit handler, and the submit handler MUST call \`event.preventDefault()\` (or \`onsubmit\` return false) before redirecting. Persist demo auth with \`window.name\` — it survives same-frame \`.html\` navigation in the sandbox — for example \`window.name="authed"; try{localStorage.setItem("authed","1")}catch(_){}\`, and have guards check \`window.name\` first. Navigate the same frame to a plain \`.html\` file such as \`index.html\` (\`location.href="index.html"\` or \`<a href="index.html">\`). Do NOT carry auth in query strings or hashes, and do NOT use \`window.top.location\`, \`window.parent.location\`, \`window.open\`, \`target="_blank"\`, \`target="_top"\`, or \`target="_parent"\` for internal auth navigation; those break the sandbox file-switch bridge.
...
function renderMetadataBlock(
  ...
  if (metadata.kind === 'prototype' || metadata.kind === 'template' || metadata.kind === 'other') {
    ...
    lines.push(
      '- **auth-flow runnable rule**: login/auth is only complete when it works across files in the preview. Put shared guard/login/logout helpers in a common JS file (for example `js/auth.js`) and load it on every page that calls them: `login.html` MUST include `<script src="js/auth.js"></script>` before its inline submit handler, and protected pages must include it before `checkAuth()` / logout bindings. `login.html` submits through a mock `handleLogin(event)` (or an equivalent mock handler) that calls `event.preventDefault()` (or returns `false` from `onsubmit`), sets demo auth, and redirects to `index.html`; protected pages check demo auth and redirect to `login.html`; logout clears it and redirects. Persist demo auth with `window.name` — it survives same-frame `.html` navigation in the sandbox — for example `window.name="authed"; try{localStorage.setItem("authed","1")}catch(_){}`, and have guards check `window.name` first. Navigate the same frame to a plain `.html` file such as `index.html` (`location.href="index.html"` or `<a href="index.html">`). Do NOT carry auth in query strings or hashes, and do NOT use `window.top.location`, `window.parent.location`, `window.open`, `target="_blank"`, `target="_top"`, or `target="_parent"` for internal auth navigation; those break the sandbox file-switch bridge.',
    );
    lines.push(
