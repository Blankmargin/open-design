...
export async function buildApiProjectContext(input: {
  ...
  const lines = [
    ...
    'For login/auth fixes, make the flow executable across files: login.html must load or define its submit handler before form submission, the submit handler must call event.preventDefault() or return false, protected pages must load the guard/logout script before guard/logout calls, and demo auth must set `window.name` before same-frame navigation. Protected pages must check `window.name` before reading localStorage/sessionStorage. Do not use URL query/hash fallbacks such as `?auth=1` or `URLSearchParams(window.location.search)` for auth; srcDoc previews strip query/hash during file navigation. Do not use window.top/window.parent/window.open/target=_blank for internal auth navigation.',
