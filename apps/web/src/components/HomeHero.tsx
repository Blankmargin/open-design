...
// composed with the recent-projects strip
...
export const HomeHero = forwardRef<HTMLTextAreaElement, Props>(function HomeHero(
  ...
  return (
    <section className="home-hero" data-testid="home-hero">
      {/*
        Hidden for now. Restore the brand row by moving this block back above
        the title:
      ...
      */}
      ...
      {/*
        Hidden for now. The tab row intentionally filters to the Prototype
        chip only. Restore all create chips by removing that filter in
        RailGroup. Restore the former "More" shortcuts by re-adding the
        ShortcutsMenu state/effects/component and passing it as children here:
...
function RailGroup({
  ...
  const chips = useMemo(
    () => {
      const groupChips = chipsForGroup(group);
      // Hidden for now: the other create chips still exist in
      // `home-hero/chips.ts`; remove this filter to restore the full row.
      return variant === 'tabs'
        ? groupChips.filter((chip) => chip.id === 'prototype')
        : groupChips;
    },
    [group, variant],
  );
